<x-app-layout>
    <div class="py-12">
        <div class="content mt-3">
            <div class="animated fadeIn">
                <div class="row mx-auto sm:px-6 lg:px-8">
                    <div class="col-4 bg-white overflow-hidden shadow-xl sm:rounded-lg">
                        @yield('content1')
                    </div>
                    <div class="col-4 bg-white overflow-hidden shadow-xl sm:rounded-lg">
                        @yield('content2')
                    </div>
                </div>
            </div>
        </div>
        <div class="row max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-jet-button class="border border-gray-300 text-black font-bold py-2 px-4 my-2 rounded">{{ __('Note') }}</x-jet-button>
            <x-jet-button class="border border-gray-300 text-black font-bold py-2 px-4 my-2 rounded">{{ __('Quote') }}</x-jet-button>
        </div>
    </div>
</x-app-layout>
